    ...
    return (
        <header className="relative min-h-[78vh] grid place-items-center overflow-hidden" role="banner" aria-label="Header">
                ...
                <div className="flex gap-2 items-center" role="group" aria-label="Theme and accessibility">
                    <label className="relative" title="Brand theme">
                        ...
                        <select
                            ...
                            className="appearance-none bg-white/10 text-[--ink] border border-white/20 rounded-lg py-2 px-4"
                        >
                    ...
                    <Button variant="ghost" onClick={() => setIsTtsOn(!isTtsOn)} aria-pressed={isTtsOn} aria-label="Read text aloud">{isTtsOn ? 'Reading' : 'Read'}</Button>
                    <Button variant="ghost" onClick={() => setIsDarkMode(!isDarkMode)} aria-pressed={isDarkMode} aria-label="Toggle dark/light">Theme</Button>
                </div>
                <a className="hidden md:inline-block text-center no-underline font-semibold rounded-[0.95rem] py-3 px-4 border-2 border-transparent bg-gradient-to-tr from-[--a] via-[--b] to-[--c] text-[#09101a]" href="#cta">Contact</a>
            </nav>
            ...
            <div className="relative max-w-4xl text-center p-[clamp(1rem,3vw,2rem)] my-8 mx-auto">
                ...
                <p className="lead text-[clamp(1.05rem,2.2vw,1.25rem)] text-[--muted] mt-2 hero-anim hero-anim-2">Design signs that breathe, windows that glow, and façades that move together — all from a calm, simple app.</p>
                <div className="hero-cta flex gap-4 justify-center flex-wrap mt-3 hero-anim hero-anim-3">
                    ...
                    <a className="inline-block text-center no-underline font-semibold rounded-[0.95rem] py-3 px-4 border-2 border-[--c] bg-transparent" href="#led-node">Try LED control</a>
                </div>
