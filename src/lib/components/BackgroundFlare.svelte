<script>
  ...
  onMount(() => {
    ...
    function flare(time) {
      t = time * 0.001;
      // ease toward pointer with inertia
      hx += (targetX - hx) * 0.06;
      hy += (targetY - hy) * 0.06;
      ...
      if (!reduced) requestAnimationFrame(flare);
    }
...
<canvas
  ...
  id="backgroundFlare"
  aria-hidden="true"
  style="position: fixed; inset: 0; width: 100%; height: 100%; display: block; pointer-events: none; z-index: -4;"
></canvas>
